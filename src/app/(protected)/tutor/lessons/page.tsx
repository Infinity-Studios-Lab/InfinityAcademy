...
import { getLessonsForTutor } from "@/lib/db/lessons";
...
export default async function TutorLessonsPage({
  ...
  const lessons = await getLessonsForTutor(user.id);
  ...
  const calendarEvents = lessons.map((lesson) => ({
    id: lesson.id,
    title: `${lesson.title} - ${lesson.student?.profile_data?.firstName || lesson.student?.email || "Student"}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Link href="/tutor/schedule" className="btn btn-primary">
          Schedule New Lesson
        </Link>
      </div>
                    ...
                    return (
                      <div key={lesson.id} className="border-l-4 border-primary pl-4">
                        ...
                        <p className="text-sm text-base-content/70">
                          {lesson.student?.profile_data?.firstName || lesson.student?.email}
                        </p>
                        <p className="text-sm text-base-content/70">
                ...
                {lessons.map((lesson) => {
                  ...
                  return (
                    <tr key={lesson.id}>
                      <td>{lesson.title}</td>
                      <td>
                        {lesson.student?.profile_data?.firstName && lesson.student?.profile_data?.lastName
                          ? `${lesson.student.profile_data.firstName} ${lesson.student.profile_data.lastName}`
                          : lesson.student?.email || "Unknown"}
                      </td>
